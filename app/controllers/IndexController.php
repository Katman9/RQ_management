...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("user_id", $username)->orWhere("email_address", $username);
		...
		if(!empty($user)){
			...
			if(password_verify($password_text,$password_hash)){
				...
				if($rememberme == true){
					...
					$db->where("employee_id", $user['employee_id']);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("employee_id","first_name","surname","gender","user_id","password","email_address","phone_no","department_name","account_status"); //registration fields
			...
			if($this->validated()){
				...
				if($rec_id){
					$this->login_user($modeldata['email_address'] , $password_text);
					return;
				}
				else{
					$this->set_page_error();
